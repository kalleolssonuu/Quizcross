  ...
  <header>
    <div>
      <Modal v-bind:uiLabels="uiLabels" v-bind:lang="lang" v-bind:sourceName="sourceName"
        v-on:switchLanguage="switchLanguage" style="position: fixed">
...
<!--       <input v-if="this.lang == 'en'" type="text" id="wordInput" v-model="word" required="required"
 ...
 -->
    ...
    <button v-if="this.enableWordButtons || this.word == '' || this.desc == ''" class="standardButton disabled"
      disabled>
      {{ uiLabels.addWord }} </button>
    ...
    <div class="solutionsWrapper">
      ...
      <div id="confirmDiscardButtons">
        <button v-if="!this.enableWordButtons" class="standardButton disabled" disabled style="width: 7vw;">
          {{ uiLabels.discard }} </button>
        <button v-else v-on:click="this.discardWord" class="standardButton" style="width: 7vw;"> {{ uiLabels.discard }}
        </button>

        <button v-if="!this.enableWordButtons" class="standardButton disabled" disabled style="width: 7vw;">
          {{ uiLabels.confirm }} </button>
        <button v-else v-on:click="this.confirmWord" class="standardButton" style="width: 7vw;"> {{ uiLabels.confirm }}
        </button>
      </div>
    </div>
  ...
  <div id="div4">
    <Crossword v-bind:sourceName="sourceName" v-bind:crossword="this.crossword.actual.posList"
  ...
  <div class="modalDescription">
    <div class="overlay" v-if="showModalDescription" @click="showModalDescription = false">
    </div>
...
export default {
  ...
  methods: {
    ...
    findPotentialMatches: function () {
      ...
      const horiz = this.cellsAmount;
      const vert = this.cellsAmount;
      ...
      for (let h = 0; h < horiz; h++) {
        for (let v = 0; v < vert; v++) {
          ...
          if (this.crossword.actual.posList[v][h].letter === wordSplit[0] || this.crossword.actual.posList[v][h].letter === null) {
            ...
            if (wordSplit.length <= horiz - h) {

              for (let ih = 0; ih < wordSplit.length; ih++) {

                if ((this.crossword.actual.posList[v][h + ih].letter === wordSplit[ih]) || (this.crossword.actual.posList[v][h + ih].letter === null)) {
    ...
    fillPositionsNull: function () {
      for (let v = 0; v < this.cellsAmount; v++) {
        ...
        for (let h = 0; h < this.cellsAmount; h++) {
          this.crossword.actual.posList[v][h] = {
            letter: null,
            inHorizontal: false,
            inVertical: false,
            isFirstLetter: false,
